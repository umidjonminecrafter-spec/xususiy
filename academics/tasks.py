...
def send_daily_telegram_reports():
    ...
    # Har kuni ertalab 7 kundan buyon qabul qilinmagan murojaatlarni ham tekshirib eslatma yuboramiz
    try:
...
def check_and_escalate_unresolved_appeals():
    """
    7 kundan buyon qabul qilinmagan (status='pending') va tashkilot egasiga hali
    yuborilmagan (is_escalated_to_owner=False) murojaatlarni aniqlab,
    tashkilot egasining Hisobot botiga (@smarttalim_report_bot) ogohlantirish yuboradi.
    """
    from django.db import connection
    connection.close()
    ...
    overdue_appeals = StudentAppeal.objects.filter(
        status='pending',
    ...
    for appeal in overdue_appeals:
        ...
        if not token:
            print(f"[APPEAL_ESCALATION] Token topilmadi, org={org.name}")
            continue

        owner_chat_ids = set()

        # 1. role='owner' bo'lgan userlar
        ...
        # 2. Agar owner topilmasa, tashkilot superuser/admin userlari
        ...
        # 3. TelegramNotificationSetting.chat_ids
        try:
        ...
        if not owner_chat_ids:
            print(f"[APPEAL_ESCALATION] Tashkilot egasi chat ID si topilmadi: {org.name}")
            continue
        ...
        msg = (
            f"⚠️ <b>DIQQAT: 7 KUNDAN BUYON QABUL QILINMAGAN MUROJAAT!</b>\n\n"
            f"Hurmatli rahbar, quyidagi murojaat tizim bildirishnomalariga (Notification) kelgan, "
            f"ammo <b>7 kun ichida</b> ma'muriyat tomonidan qabul qilinmagan yoki ko'rib chiqilmagan:\n\n"
            f"👤 <b>Talaba:</b> {student_name}\n"
            f"📞 <b>Telefon:</b> {appeal.student.phone}\n"
            f"🏢 <b>Tashkilot:</b> {org.name}\n"
            f"📌 <b>Murojaat turi:</b> {type_display}\n"
            f"📅 <b>Kelgan sana:</b> {created_str} (7+ kun oldin)\n\n"
            f"💬 <b>Murojaat matni:</b>\n"
            f"<i>\"{appeal.message}\"</i>\n\n"
            f"⚠️ <i>Iltimos, ushbu masala bo'yicha zudlik bilan chora ko'ring!</i>"
        )
        ...
        for chat_id in owner_chat_ids:
            try:
                if send_telegram_message(token, chat_id, msg):
                    sent_any = True
                    print(f"[APPEAL_ESCALATED] Appeal #{appeal.id} sent to owner chat_id={chat_id}")
            except Exception as e_send:
                print(f"[APPEAL_ESCALATION_ERR] chat_id={chat_id}: {e_send}")

        if sent_any:
            appeal.is_escalated_to_owner = True
            appeal.escalated_at = timezone.now()
            appeal.save(update_fields=['is_escalated_to_owner', 'escalated_at'])
            escalated_count += 1
